...
const RenderRequestCards: React.FC<requestCardData[]> = (
  ...
  return (
    <div className="RHomeDetails-application">
      <Row
        ...
        className="RHomeDetails-application-section"
        gutter={[10, 10]}>
        <h2 className="RHomeDetails-application-section-title">
          Candidates ({requestData.length})
        </h2>
        {requestData.map((item: requestCardData, i: number) => (
          // Edited by William. For better display and hover effects
          <Card.Grid
            className="RHomeDetails-card-block"
            key={i.toString()}
            hoverable={true}>
            <RequestCard
              logo={item.logo}
